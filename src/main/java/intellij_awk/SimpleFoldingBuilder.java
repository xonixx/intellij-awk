...
public class SimpleFoldingBuilder extends FoldingBuilderEx implements DumbAware {
   ...
   *     SIMPLE_PREFIX_STR + SIMPLE_SEPARATOR_STR + Key, where Key is defined by the Simple language
   *     file.
   */
  @Nullable
  @Override
  public String getPlaceholderText(@NotNull ASTNode node) {
    ...
    if (node.getPsi() instanceof PsiLiteralExpression) {
      ...
      String key =
          ((String) nodeElement.getValue())
              .substring(
                  SimpleAnnotator.SIMPLE_PREFIX_STR.length()
                      + SimpleAnnotator.SIMPLE_SEPARATOR_STR.length());
      final List<SimpleProperty> properties =
          SimpleUtil.findProperties(nodeElement.getProject(), key);
